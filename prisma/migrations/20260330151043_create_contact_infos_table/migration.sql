...
-- AlterTable
ALTER TABLE `profiles` ADD COLUMN `avatar_id` VARCHAR(191) NULL,
    ADD COLUMN `cover_photo_id` VARCHAR(191) NULL,
...
-- AddForeignKey
ALTER TABLE `profiles` ADD CONSTRAINT `profiles_avatar_id_fkey` FOREIGN KEY (`avatar_id`) REFERENCES `media`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `profiles` ADD CONSTRAINT `profiles_cover_photo_id_fkey` FOREIGN KEY (`cover_photo_id`) REFERENCES `media`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
